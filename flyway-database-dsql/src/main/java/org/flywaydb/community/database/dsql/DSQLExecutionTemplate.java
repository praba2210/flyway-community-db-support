 ...
 * being re-declared here; the {@code dsql} configuration supplies the count and cap as explicit
 * overrides on top of the connector defaults (the count defaults to 0, so OCC retry is off unless
 * {@code flyway.dsql.occMaxRetries} is set to a positive value).
 */
class DSQLExecutionTemplate implements ExecutionTemplate {

    private static final Logger LOG = Logger.getLogger(DSQLExecutionTemplate.class.getName());

    private final ExecutionTemplate delegate;
    private final OCCRetryConfig retryConfig;

    DSQLExecutionTemplate(ExecutionTemplate delegate, OCCRetryConfig retryConfig) {
